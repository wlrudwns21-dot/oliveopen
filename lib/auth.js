...
/** 비밀번호 검증. { ok, legacy } — legacy=true면 sha256 계정이라 bcrypt로 재해싱 필요 */
export async function verifyPassword(pw, stored) {
  if (!stored) return { ok: false, legacy: false };
  if (stored.startsWith('$2')) {
    return { ok: await bcrypt.compare(String(pw), stored), legacy: false };
  }
  const ok = sha256(pw) === stored;
  return { ok, legacy: ok }; // 옛 sha256 계정: 맞으면 재해싱 대상
}
