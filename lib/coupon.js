/** 쿠폰 사용 가능 여부 판정 (조건: 활성·기간·발급한도·최소주문·회원등급·특정상품·추천코드) */
export function couponUsable(c, { total = 0, grade = null, cartProductPks = [], referralCode = null } = {}) {
  ...
  // 추천 쿠폰은 그 코드로 가입한 회원만 사용 가능 (전체 공개 아님)
  if (c.is_referral && c.code !== referralCode) return false;
  return true;
}
